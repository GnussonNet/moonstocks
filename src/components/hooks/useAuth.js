...
export const AuthProvider = ({ children }) => {
  ...
  const isSignedIn = async () => {
    try {
      const res = await axios.post('https://stocks.gnusson.net/api/user/is_signed_in');
  ...
  const signinWithJwtRefreshToken = async () => {
    try {
      const res = await axios.post('https://stocks.gnusson.net/api/user/refresh_token');
  ...
  const signinWithEmailAndPassword = async (email, password) => {
    try {
      const res = await axios.post('https://stocks.gnusson.net/api/user/signin', {
        email: email,
        password: password,
      });
  ...
  const createAccountWithEmailAndPassword = async (firstName, lastName, email, password) => {
    try {
      const res = await axios.post('https://stocks.gnusson.net/api/user/create_account', {
        firstName: firstName,
        lastName: lastName,
        email: email,
        password: password,
  ...
  const logout = async () => {
    try {
      await axios.post('https://stocks.gnusson.net/api/user/logout');
      // Return true of successful
      return true;
    } catch (err) {
      // Return false and log error code if any
